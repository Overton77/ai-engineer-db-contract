...
-- This serializer is deliberately limited to the integer-only receipt shape
-- constructed below. It is not a replacement for general JSON canonicalization.
create function orchestration.structured_extraction_receipt_json(value jsonb)
returns text language plpgsql immutable strict set search_path='' as $$
declare result text;
begin
 case jsonb_typeof(value)
 when 'object' then
  select '{'||coalesce(string_agg(to_jsonb(e.key)::text||':'||orchestration.structured_extraction_receipt_json(e.value),',' order by e.key collate "C"),'')||'}'
   into result from jsonb_each(value) e;
 when 'array' then
  select '['||coalesce(string_agg(orchestration.structured_extraction_receipt_json(e.value),',' order by e.ordinal),'')||']'
   into result from jsonb_array_elements(value) with ordinality e(value,ordinal);
 when 'number' then
  if value::text !~ '^-?(0|[1-9][0-9]*)$' or abs((value::text)::numeric)>9007199254740991 then
   raise exception 'structured extraction receipt requires safe integers' using errcode='check_violation';
  end if;
  result:=value::text;
...
create function orchestration.structured_extraction_result_body(tenant uuid,operation uuid)
...
-- Replace the temporary denial in-place so its two installed triggers continue
-- to guard both initial inserts and transitions; old migration source is intact.
create or replace function orchestration.verification_structured_extraction_completion_guard()
...
begin
 ...
 claim:=nullif(current_setting('verification.structured_extraction_terminal_claim',true),'')::jsonb;
 expected:=orchestration.structured_extraction_result_body(new.tenant_id,target_operation_id);
 ...
 if tg_table_name='operation_step' then
  ...
  select count(*) into matches from knowledge_service.receipt r join knowledge_service.operation_step s on s.tenant_id=r.tenant_id and s.id=r.step_id
   where r.tenant_id=new.tenant_id and r.operation_id=target_operation_id and r.outcome='succeeded' and r.receipt_kind='extract_and_register.succeeded'
    and r.id::text=claim->>'receiptId' and r.idempotency_key=claim->>'idempotencyKey' and r.executor_identity=claim->>'executorIdentity'
    and s.operation_id=target_operation_id and s.step_key='extract_and_register' and s.status='succeeded' and s.id::text=claim->>'stepId'
    and r.input_sha256=s.input_sha256 and r.output_sha256=expected_sha and (r.body-array['eventId','fencingToken'])=expected;
  if matches<>1 then raise exception 'structured extraction operation success requires one exact receipt' using errcode='foreign_key_violation'; end if;
 end if;
 return new;
end $$;

create function orchestration.guard_structured_extraction_success_receipt()
returns trigger language plpgsql set search_path='' as $$
...
begin
 ...
 if kind is distinct from 'verification_structured_extraction' or new.outcome is distinct from 'succeeded' then return new; end if;
 claim:=nullif(current_setting('verification.structured_extraction_terminal_claim',true),'')::jsonb;
 expected:=orchestration.structured_extraction_result_body(new.tenant_id,new.operation_id);
 expected_sha:=encode(extensions.digest(convert_to(orchestration.structured_extraction_receipt_json(expected),'UTF8'),'sha256'),'hex');
 if claim is null or expected is null or new.receipt_kind is distinct from 'extract_and_register.succeeded' or new.step_id is null
  ...
  or not exists(select 1 from knowledge_service.operation_step s join knowledge_service.lease l on l.tenant_id=s.tenant_id and l.operation_step_id=s.id
   where s.tenant_id=new.tenant_id and s.id=new.step_id and s.operation_id=new.operation_id and s.status='succeeded' and s.step_key='extract_and_register'
  ...
  or not exists(select 1 from knowledge_service.operation_event e where e.tenant_id=new.tenant_id and e.id::text=new.body->>'eventId'
   and e.operation_id=new.operation_id and e.step_id=new.step_id and e.event_kind='step.succeeded' and e.from_state='running' and e.to_state='succeeded'
  ...
  or exists(select 1 from knowledge_service.receipt r where r.tenant_id=new.tenant_id and r.operation_id=new.operation_id and r.step_id=new.step_id and r.outcome='succeeded') then
  raise exception 'structured extraction success receipt binding mismatch' using errcode='foreign_key_violation';
 end if;
 return new;
end $$;
create trigger verification_structured_extraction_success_receipt before insert on knowledge_service.receipt
 for each row execute function orchestration.guard_structured_extraction_success_receipt();
...
create function orchestration.guard_structured_extraction_terminal_commit()
returns trigger language plpgsql set search_path='' as $$
declare expected jsonb; matches bigint;
begin
 if new.status is distinct from 'succeeded' or not exists(select 1 from knowledge_service.operation o where o.tenant_id=new.tenant_id and o.id=new.operation_id and o.operation_kind='verification_structured_extraction') then return null; end if;
 expected:=orchestration.structured_extraction_result_body(new.tenant_id,new.operation_id);
 select count(*) into matches from knowledge_service.operation o join knowledge_service.receipt r on r.tenant_id=o.tenant_id and r.operation_id=o.id
  where o.tenant_id=new.tenant_id and o.id=new.operation_id and o.status='succeeded' and r.step_id=new.id
   and r.outcome='succeeded' and r.receipt_kind='extract_and_register.succeeded' and (r.body-array['eventId','fencingToken'])=expected;
...
create constraint trigger verification_structured_extraction_terminal_commit after insert or update on knowledge_service.operation_step
 deferrable initially deferred for each row execute function orchestration.guard_structured_extraction_terminal_commit();
commit;
