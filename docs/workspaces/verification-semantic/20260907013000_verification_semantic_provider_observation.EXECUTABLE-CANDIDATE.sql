...
declare c jsonb; p orchestration.verification_provider_attempt%rowtype; envelope uuid;
begin
 ...
 if c is null or not orchestration.verification_provider_scope_tuple_is_live(new.tenant_id,new.operation_id,new.operation_step_id,new.profile_artifact_id,new.profile_sha256,c) then raise exception 'semantic observation stale closed-scope lease' using errcode='check_violation'; end if;
 ...
 if not found or p.operation_id<>new.operation_id or p.operation_step_id<>new.operation_step_id or p.profile_artifact_id<>new.profile_artifact_id or p.profile_sha256<>new.profile_sha256 or p.dispatch_fencing_token<>new.dispatch_fencing_token or p.request_artifact_id<>new.request_artifact_id or p.request_sha256<>new.request_sha256 or p.state not in('dispatched','uncertain','settled') then raise exception 'semantic observation provider attempt binding mismatch' using errcode='foreign_key_violation'; end if;
 ...
 select m.artifact_id into envelope from orchestration.verification_artifact_metadata m where m.tenant_id=new.tenant_id and m.parent_artifact_ids=array[new.request_artifact_id,new.raw_response_artifact_id] limit 1;
 if envelope is null or not orchestration.verification_artifact_is_admitted(new.tenant_id,new.raw_response_artifact_id,'verification_provider_raw_response',new.raw_response_sha256) or not exists(select 1 from orchestration.verification_artifact_metadata m where m.tenant_id=new.tenant_id and m.artifact_id=new.observation_artifact_id and m.parent_artifact_ids=array[new.blinded_input_artifact_id,envelope,new.profile_artifact_id]) then raise exception 'semantic observation exact artifact closure mismatch' using errcode='foreign_key_violation'; end if;
 return new;
end $$;
