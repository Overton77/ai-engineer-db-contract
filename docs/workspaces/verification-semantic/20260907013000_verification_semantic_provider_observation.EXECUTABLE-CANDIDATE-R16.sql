...
-- R16 only extends the existing reconciliation guard. Extraction v1 retains its exact R15/R33000 branch.
...
begin
 ...
 if b->>'schemaVersion'<>'verification-semantic-provider-reconciliation.v1' then
 ...
 if not coalesce(
  p.tenant_id=new.tenant_id and p.operation_id=o.id and p.operation_step_id=(b->>'operationStepId')::uuid
  and p.state in('dispatched','uncertain') and p.actual_cost_micros is null and p.budget_id=budget.id
  ...
  and (b->>'issuedAt')::timestamptz<=clock_timestamp() and (b->>'expiresAt')::timestamptz>clock_timestamp()
  and (b->>'expiresAt')::timestamptz>(b->>'issuedAt')::timestamptz and (b->>'expiresAt')::timestamptz<=(b->>'issuedAt')::timestamptz+interval '24 hours'
  and exists(select 1 from knowledge_service.operation_step s where s.tenant_id=o.tenant_id and s.id=p.operation_step_id and s.operation_id=o.id
 ...
 if b ? 'capture' then
  if not (b->'capture' ?& array['transportArtifact','responseEnvelopeArtifact','rawResponseArtifact']) or jsonb_object_length(b->'capture')<>3
   or not orchestration.provider_reconciliation_handle_matches(new.tenant_id,b#>'{capture,transportArtifact}')
   or not orchestration.provider_reconciliation_handle_matches(new.tenant_id,b#>'{capture,responseEnvelopeArtifact}')
   or not orchestration.provider_reconciliation_handle_matches(new.tenant_id,b#>'{capture,rawResponseArtifact}') then
   raise exception 'semantic provider reconciliation capture handle mismatch' using errcode='foreign_key_violation'; end if;
  select * into capture from orchestration.verification_provider_response_capture c where c.tenant_id=new.tenant_id and c.provider_attempt_id=p.id for update;
  if not found or capture.operation_id<>o.id or capture.operation_step_id<>p.operation_step_id or capture.profile_artifact_id<>p.profile_artifact_id
   or capture.profile_sha256<>p.profile_sha256 or capture.dispatch_fencing_token<>p.dispatch_fencing_token
   or capture.transport_artifact_id<>(b#>>'{capture,transportArtifact,artifactId}')::uuid
   or capture.transport_sha256<>substring(b#>>'{capture,transportArtifact,digest}' from 8)
   or capture.response_envelope_artifact_id<>(b#>>'{capture,responseEnvelopeArtifact,artifactId}')::uuid
   or capture.response_envelope_sha256<>substring(b#>>'{capture,responseEnvelopeArtifact,digest}' from 8)
   or not orchestration.verification_artifact_is_admitted(new.tenant_id,capture.transport_artifact_id,'verification_provider_transport_response',capture.transport_sha256)
   or not orchestration.verification_artifact_is_admitted(new.tenant_id,capture.response_envelope_artifact_id,'verification_provider_response_envelope',capture.response_envelope_sha256)
   or not orchestration.verification_artifact_is_admitted(new.tenant_id,(b#>>'{capture,rawResponseArtifact,artifactId}')::uuid,'verification_provider_raw_response',substring(b#>>'{capture,rawResponseArtifact,digest}' from 8))
   or not exists(select 1 from orchestration.verification_artifact_metadata m where m.tenant_id=new.tenant_id and m.artifact_id=capture.response_envelope_artifact_id and m.parent_artifact_ids=array[p.request_artifact_id,(b#>>'{capture,rawResponseArtifact,artifactId}')::uuid]) then
  ...
  if b ? 'observationArtifact' then
   if not orchestration.provider_reconciliation_handle_matches(new.tenant_id,b->'observationArtifact') then raise exception 'semantic provider reconciliation observation handle mismatch' using errcode='foreign_key_violation'; end if;
   select * into observation from orchestration.verification_semantic_response_observation so where so.tenant_id=new.tenant_id and so.provider_attempt_id=p.id for update;
   if not found or observation.operation_id<>o.id or observation.operation_step_id<>p.operation_step_id
    or observation.profile_artifact_id<>p.profile_artifact_id or observation.profile_sha256<>p.profile_sha256
    or observation.blinded_input_artifact_id<>(b#>>'{blindedInputArtifact,artifactId}')::uuid
    or observation.request_artifact_id<>p.request_artifact_id or observation.request_sha256<>p.request_sha256
    or observation.raw_response_artifact_id<>(b#>>'{capture,rawResponseArtifact,artifactId}')::uuid
    or observation.response_envelope_artifact_id<>(b#>>'{capture,responseEnvelopeArtifact,artifactId}')::uuid
    or observation.observation_artifact_id<>(b#>>'{observationArtifact,artifactId}')::uuid then
 ...
 elsif b ? 'observationArtifact' or exists(select 1 from orchestration.verification_provider_response_capture c where c.tenant_id=new.tenant_id and c.provider_attempt_id=p.id)
