---
id: "rel:corpus.dataset"
kind: table
schema: corpus
name: dataset
domain: identity
aliases: []
tokens: [corpus, dataset, corpus.dataset, id, tenant_id, kind, dataset_kind, canonical_url, license_code]
summary: null
summary_basis: none
rls: enabled
readers: [control_plane, executor_service, pipeline_agent, service_role, verifier_agent]
writers: [executor_service, service_role]
typescript: "Database[\"corpus\"][\"Tables\"][\"dataset\"][\"Row\"]"
...
| 3 | `kind` | `text` | no | `'dataset'::text` | — |
| 4 | `dataset_kind` | `text` | yes | — | — |
| 5 | `canonical_url` | `text` | yes | — | — |
| 6 | `license_code` | `text` | yes | — | FK → [`corpus.license`](license.md).code |

## Constraints

- PK (id)
- unique (tenant_id, id)
- check `dataset_kind_check`: `(kind = 'dataset'::text)`

## Relationships

Outbound: `id` → [`corpus.entity`](entity.md)`.id` (+tenant); `license_code` → [`corpus.license`](license.md)`.code`; `tenant_id,id,kind` → [`corpus.entity`](entity.md)`.tenant_id,id,kind` (deferrable).
Inbound: none.
...
`dataset_tenant_id_id_key` unique
...
insert: `Database["corpus"]["Tables"]["dataset"]["Insert"]`; row: `Database["corpus"]["Tables"]["dataset"]["Row"]`; update: `Database["corpus"]["Tables"]["dataset"]["Update"]`
